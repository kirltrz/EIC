...
public:
    ...
    LGFX(void)
    {
        {                                      // 进行总线控制的设定。
            ...
            // 配置Spi总线
            cfg.spi_host = SCREEN_SPI_HOST; // 选择要使用的SPI ESP32-S2，C3: spi_host or spi_host/ESP32: VSPI_HOST or HSPI_HOST
            ...
            cfg.pin_sclk = PIN_SCREEN_CLK;          // 设定Spi的sclk引脚编号
            cfg.pin_mosi = PIN_SCREEN_MOSI;         // 设定Spi的mosi引脚编号
            cfg.pin_miso = PIN_SCREEN_MISO;         // 设定SPI的MISO引脚编号 (-1 = disable)
            cfg.pin_dc = PIN_SCREEN_DC;             // 设置SPI的D/C引脚编号 (-1 = disable)
        ...
        {                                        // 进行显示面板控制的设定。
            ...
            cfg.pin_cs = PIN_SCREEN_CS;   // CS所连接的引脚编号 (-1 = disable)
            cfg.pin_rst = PIN_SCREEN_RST; // RST所连接的引脚编号 (-1 = disable)
        ...
        { // 进行触摸屏控制的设定。(不需要的话删除)
            ...
            cfg.pin_int = PIN_TOUCH_INT; // 连接有Int的引脚编号
            cfg.bus_shared = true;       // 使用与画面共通的总线时设定true
            cfg.offset_rotation = 0;     // 显示和触摸方向不一致时的调整0 ~ 7的值设定

            // 对于Spi连接
            cfg.spi_host = SCREEN_SPI_HOST; // 选择要使用的SPI (HSPI_HOST or VSPI_HOST)
            cfg.freq = 1000000;             // 设定Spi时钟
            cfg.pin_sclk = PIN_SCREEN_CLK;  // Sclk所连接的引脚编号
            cfg.pin_mosi = PIN_SCREEN_MOSI; // 连接有Mosi的引脚编号
            cfg.pin_miso = PIN_SCREEN_MISO; // 连接Miso的引脚编号
            cfg.pin_cs = PIN_TOUCH_CS;      //   Cs所连接的引脚编号
