...
void FSUS_Servo::setAngle(FSUS_SERVO_ANGLE_T angle, FSUS_INTERVAL_T interval, FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec, FSUS_POWER_T power){
    ...
    // 将真实角度转换为原始角度 (经过标定)
    // 注意: 这里的 rawAngleToSet 仍然是度数值，协议层会再将其乘以10
    FSUS_SERVO_ANGLE_T rawAngleToSet = angleReal2Raw(this->targetAngle);
...
// 控制模式停止指令
void FSUS_Servo::StopOnControlMode(uint8_t method, uint16_t power){
    this->protocol->sendStopOnControlMode(this->servoId, method, power);
}

// 控制模式停止指令-卸力（失锁）
void FSUS_Servo::StopOnControlUnloading(){
    this->protocol->sendStopOnControlUnloading(this->servoId);
}

// 控制模式停止指令-锁力
void FSUS_Servo::StopOnControlKeep(uint16_t power){
    this->protocol->sendStopOnControlKeep(this->servoId,power);
}

// 控制模式停止指令-阻尼
void FSUS_Servo::StopOnControlDammping(uint16_t power){
    this->protocol->sendStopOnControlDammping(this->servoId, power);
}
// 同步控制
void FSUS_Servo::SyncCommand(uint8_t servocount, uint8_t syncmode, FSUS_sync_servo servoSync[]){
        this->protocol->sendSyncCommand(servocount, syncmode, servoSync);
}
// 同步数据监控
void FSUS_Servo::SyncMonitorCommand(uint8_t servocount, FSUS_sync_servo servoSync[],ServoMonitorData* data){
        ...
        // 发送同步命令，告诉舵机们开始响应
        this->protocol->sendSyncMonitor(servocount, servoSync);

        // 调用recvsyncMonitor函数接收多个舵机的数据
        this->protocol->recvsyncMonitor(servocount,data);
        // 如果接收失败，返回nullptr
// 检查数据接收状态
    if (this->protocol->responsePack.recv_status != FSUS_STATUS_SUCCESS) {
        // 如果接收失败，标记数据为无效并返回
...
ServoMonitorData FSUS_Servo::ServoMonitor() {
    ...
    // 接收舵机监控数据
    data = this->protocol->recvServoMonitor();  // 获取结构体数据
...
void FSUS_Servo::BeginAsync(){
    this->protocol->sendBeginAsync();
}
// 结束异步指令
void FSUS_Servo::EndAsync(uint8_t cancel){
    this->protocol->sendEndAsync(cancel);
}
